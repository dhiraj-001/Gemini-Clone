...
const Voice = () => {
  const { ismicOn, offMIc, transcript, listening, resetTranscript } =
    useContext(Context);
  const {
  ...
  return (
    <div
      className={`absolute border-t-[1px] p-6 ${
        !ismicOn ? "hidden" : ""
      } w-full h-[50vh] z-10 backdrop-blur-lg rounded-2xl bottom-0 border-[#8267cf] animate-slide-up `}
    >
      <span className="">
      ...
      >
        <FaStopCircle className="text-4xl fill-[#d7bbfe]" />
      </button>
      {/* <button 
      ...
      <p className="text-[#ffffffb9] font-mini md:text-xl text-md"> {transcript}</p>
    </div>
  );
};
